...
int main(int argc, char **argv)
{
    ...
    while (time < endTime)
    {
        ...
        attitudeEuler = integrateEulerAngles(attitudeEuler, eulerAngleKinematicRates(attitudeEuler, bodyAngularRates), dt);
        ...
        {
            outputFile << time << ","
                       << attitudeDCM_Euler.phi * (180.0/M_PI) << ","
                       << attitudeDCM_Euler.theta * (180.0/M_PI) << ","
                       << attitudeDCM_Euler.psi * (180.0/M_PI) << ","
                       << attitudeEuler_Euler.phi * (180.0/M_PI) << ","
                       << attitudeEuler_Euler.theta * (180.0/M_PI) << ","
                       << attitudeEuler_Euler.psi * (180.0/M_PI) << ","
                       << attitudeQuat_Euler.phi * (180.0/M_PI) << ","
                       << attitudeQuat_Euler.theta * (180.0/M_PI) << ","
                       << attitudeQuat_Euler.psi * (180.0/M_PI)
                       << std::endl;
